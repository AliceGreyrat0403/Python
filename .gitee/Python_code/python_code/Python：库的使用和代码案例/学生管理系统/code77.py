...
def save(): # 保存操作
    ...
    # 此时这个写法的含义就是让 record.txt 和 当前的 code77.py 在同一个目录里
    with open('record.txt','w',encoding='utf8')as f:    # 直接写了文件名，没有写前面的路径(绝对路径)
        for s in students:
...
def load(): # 读档操作
    ...
    # 如果读档文件不存在，则直接跳过读档流程
    # 判断文件是否存在
    # 为了避免读方式打开文件的时候，文件不存在的情况
    if not os.path.exists('record.txt'):    # path 这个模块可以帮我们进行一些路径上的判定操作
        return
    ...
    with open('record.txt','r',encoding='utf8')as f:
        for line in f:  # 按行读取文件
            # 针对这一行数据，按照 \t 进行切分操作！
            # 切分之前要去除末尾的换行，怎么去除？
            line = line.strip() # strip这是一个非常有用的方法，它的功能是：去掉一个字符串开头和末尾的空白符
...
def main():
    ...
    print('               欢迎来到学生管理系统                ')
    print('-----------------------------------------------')
    # 需要在程序启动的时候调用 load 即可(最开始加载一次，就不用反复加载了)
    load()
    while True:
        # 通过 menu 函数来打印出菜单项
        choice = menu()
